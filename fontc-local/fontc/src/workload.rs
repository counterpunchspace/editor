...
// Simple scope that executes tasks immediately (no parallelism)
// Used when rayon is disabled for WASM compatibility
struct DummyScope;

impl DummyScope {
    fn spawn<F>(&self, func: F)
    where
        F: FnOnce(&DummyScope) + Send,
    {
        func(self);
    }
}
...
impl Workload {
    ...
    pub fn exec(mut self, fe_root: &FeContext, be_root: &BeContext) -> Result<JobTimer, Error> {
        ...
        let mut runner = |scope: &DummyScope| {
        ...
        {
            // For non-WASM, we'd use rayon but it's disabled for WASM compatibility
            // Just run sequentially for now
            runner(&DummyScope)?;
        }
        // WASM uses sequential execution
