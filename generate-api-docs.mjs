...
function tsToPythonType(tsType) {
  ...
  // Handle Array<...> types
  if (tsType.match(/^Array<.*>$/)) {
    const inner = tsType.match(/^Array<(.*)>$/)[1];
    // Recursively process the inner type
    return `list[${tsToPythonType(inner)}]`;
  }
...
function generateClassDocs(classInfo, typeDefinitions = new Map()) {
  ...
  const accessExamples = {
    ...
    Shape:
      "```python\n# Shapes are Path or Component objects\nshape = layer.shapes[0]\nif hasattr(shape, 'nodes'):\n    # Path - has nodes property\n    nodes = shape.nodes\nelif hasattr(shape, 'reference'):\n    # Component - has reference property\n    ref = shape.reference\n```",
...
function generateAPIDocs(version = null) {
  ...
  // Type definitions
  lines.push(getTypeDefinitionsSection());
...
/**
 * Get the type definitions section
 */
function getTypeDefinitionsSection() {
  return `## Type Definitions

**Note:** PossiblyAutomaticCode is now documented as a full class in the [Class Reference](#possiblyautomaticcode) section.
...
**Note:** Values are \`PossiblyAutomaticCode\` objects with a \`code\` property containing the actual feature code:
...
# Iterate over values (PossiblyAutomaticCode objects)
for code_obj in font.features.classes.values():
    print(f"Code: {code_obj['code']}")

# Iterate over key-value pairs
for name, code_obj in font.features.classes.items():
    print(f"{name}: {code_obj['code']}")
    if code_obj.get("automatic"):
        print("  (auto-generated)")

# Direct access returns PossiblyAutomaticCode object
code_obj = font.features.classes["@lowercase"]
actual_code = code_obj["code"]

# Assignment accepts plain strings (auto-converted) or objects
font.features.classes["@uppercase"] = "A B C D E"
font.features.classes["@marks"] = {"code": "A B C", "automatic": True}
\`\`\`
