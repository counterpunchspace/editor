...
- **API Documentation Generator**: Added `downloadAPIDocs()`, `showAPIDocs()`, and `generateAPIDocs()` functions to generate comprehensive Python-oriented API documentation for the Font Object Model. Documentation is auto-generated from the JavaScript classes via Pyodide introspection. Includes offline Node.js script (`npm run generate-docs`) to generate `API.md` for inclusion in the repo.
